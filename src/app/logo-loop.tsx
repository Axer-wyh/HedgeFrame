...
function isNodeLogo(item: LogoItem): item is NodeLogoItem {
  return "node" in item;
}
  ...
  useEffect(() => {
    ...
    const animate = (timestamp: number) => {
      ...
      if (seqSize > 0) {
        ...
        offsetRef.current = ((rawOffset % seqSize) + seqSize) % seqSize;
        track.style.transform = isVertical
          ? `translate3d(0, ${-offsetRef.current}px, 0)`
          : `translate3d(${-offsetRef.current}px, 0, 0)`;
      }
...
export const LogoLoop = memo(function LogoLoop({
  ...
  const refs = useMemo(() => [containerRef, seqRef], []);
  ...
  useResizeObserver(updateDimensions, refs);
